  ...
  return (
    <div className="max-w-[1440px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
      ...
      <nav className="flex items-center gap-2 text-xs text-[#736E9B] mb-4 sm:mb-6">
        ...
        <IconChevronRight className="w-3.5 h-3.5 text-[#A59FC2]" />
        <button
          onClick={handleResetFilters}
          className={`hover:text-[#FF4D6D] transition-colors font-medium ${
        ...
        {currentCategoryObj && (
          <>
            <IconChevronRight className="w-3.5 h-3.5 text-[#A59FC2]" />
            <span className="text-[#171136] font-bold">{currentCategoryObj.label}</span>
          </>
        )}
      </nav>

      {/* 2. Top Header Title & Sorting Control Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#EAE3F7] mb-6">
        <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="lg:hidden flex items-center gap-2 bg-[#F6F1FF] hover:bg-[#EFE9FF] border border-[#EAE3F7] text-[#171136] font-bold text-xs sm:text-sm px-4 py-2.5 rounded-full shadow-xs active:scale-95 transition-all"
          >
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-white border border-[#EAE3F7] text-[#171136] font-semibold text-xs sm:text-sm rounded-xl px-3.5 py-2.5 outline-none focus:border-[#FF4D6D] shadow-xs cursor-pointer"
            >
      ...
      <div className="flex flex-col lg:flex-row items-start gap-6 xl:gap-8">
        ...
        <div className="flex-1 w-full min-w-0">
          ...
          {filteredProducts.length === 0 ? (
            <div className="bg-white border border-[#EAE3F7] rounded-[24px] p-12 text-center flex flex-col items-center justify-center">
              <div className="w-16 h-16 rounded-full bg-[#FFF1F4] flex items-center justify-center mb-4">
                <IconSearch className="w-8 h-8 text-[#FF4D6D]" />
              </div>
              ...
              <button
                onClick={handleResetFilters}
                className="mt-5 bg-[#FF4D6D] hover:bg-[#ff3358] text-white font-bold text-xs sm:text-sm px-6 py-2.5 rounded-full shadow-sm active:scale-95 transition-all"
              >
                Reset All Filters
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4.5">
              {filteredProducts.map((product) => {
                ...
                return (
                  <div
                    key={product.id}
                    className="relative bg-white border border-[#EAE3F7] rounded-2xl sm:rounded-[20px] shadow-[0_12px_0_-5px_rgba(23,17,54,0.06)] hover:shadow-[0_18px_30px_-8px_rgba(23,17,54,0.12)] overflow-hidden flex flex-col group transition-all hover:-translate-y-1"
                  >
                    {/* Top Image Box */}
                    <div className="relative h-[140px] sm:h-[180px] bg-[#FAF7FF] flex items-center justify-center p-3 overflow-hidden">
                      {discountPercent ? (
                        <span className="absolute left-2.5 top-2.5 bg-[#FF4D6D] text-white text-[9.5px] sm:text-[11px] font-extrabold px-2 py-0.5 rounded-full z-10 shadow-xs">
                          -{discountPercent}%
                        </span>
                      ) : null}

                      {/* Top Right Actions (Heart + Quick View Eye) */}
                      <div className="absolute right-2 top-2 z-10 flex flex-col gap-1.5 opacity-90 group-hover:opacity-100 transition-opacity">
                        <button
                          type="button"
                          aria-label="Wishlist"
                          onClick={(e) => toggleWishlist(product.id, e)}
                          className={`w-7.5 h-7.5 rounded-full bg-white/95 backdrop-blur-md flex items-center justify-center shadow-xs transition-colors ${
                        ...
                        <button
                          ...
                          className="w-7.5 h-7.5 rounded-full bg-white/95 backdrop-blur-md flex items-center justify-center shadow-xs text-[#736E9B] hover:text-[#7B5CFF] transition-colors"
                        >
                      ...
                      >
                        <Image
                          ...
                          width={140}
                          height={160}
                          className="w-[75px] sm:w-[110px] h-auto transition-transform group-hover:scale-105 drop-shadow-sm"
                        />
                      </Link>
                    </div>
                    ...
                    <div className="p-3 sm:p-4 flex flex-col flex-1 justify-between gap-2.5">
                      <div>
                        <span
                          className="text-[9px] sm:text-[10px] font-bold tracking-wide uppercase block"
                        ...
                        <Link
                          href={`/product/${product.slug || product.id}`}
                          className="font-[family-name:var(--font-display)] font-extrabold text-[12.5px] sm:text-[14.5px] text-[#171136] hover:text-[#FF4D6D] transition-colors line-clamp-2 mt-0.5 leading-snug"
                        >
                          {product.name}
                        </Link>
                      </div>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[10px] sm:text-[11px] text-[#736E9B] font-medium ml-1">
                          ({product.reviews ?? 0})
                        </span>
                      </div>

                      {/* Price & Action Button */}
                      <div className="pt-2 border-t border-[#F0EBF9] flex items-center justify-between gap-1 mt-auto">
                        <div className="flex flex-col">
                          <span className="font-[family-name:var(--font-display)] font-extrabold text-[13.5px] sm:text-[16px] text-[#171136] leading-none">
                            {discountedPrice}
                          </span>
                          {regularPrice && regularPrice !== discountedPrice && (
                            <span className="text-[10.5px] text-[#736E9B] line-through font-medium mt-0.5">
                              {regularPrice}
                            </span>
                          )}
                        </div>
      ...
      {quickViewProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          />
          <div className="relative z-10 bg-white rounded-[28px] max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-[#EAE3F7] animate-in zoom-in-95 duration-200 flex flex-col sm:flex-row gap-6">
            <button
              onClick={() => setQuickViewProduct(null)}
              className="absolute top-4 right-4 w-9 h-9 rounded-full bg-[#F6F1FF] hover:bg-[#EFE9FF] flex items-center justify-center text-[#171136] cursor-pointer"
            >
              <IconX className="w-4 h-4" />
            </button>

            <div className="sm:w-1/2 h-[220px] sm:h-auto bg-[#FAF7FF] rounded-2xl flex items-center justify-center p-4">
              <Image
                src={quickViewProduct.image || "/images/figure-samurai-red.svg"}
                alt={quickViewProduct.name}
                width={180}
                height={220}
                className="max-h-[190px] w-auto drop-shadow-md"
              />
            </div>
            ...
            <div className="sm:w-1/2 flex flex-col justify-between">
              <div>
                ...
                <h3 className="font-[family-name:var(--font-display)] font-extrabold text-xl text-[#171136] mt-1 leading-tight">
                  {quickViewProduct.name}
                </h3>
                <p className="text-xs text-[#736E9B] mt-2 line-clamp-3 leading-relaxed">
                  {quickViewProduct.description || "Authentic limited collector item from the Brickverse universe."}
                </p>
                ...
                <div className="mt-4 flex items-baseline gap-2">
                  ...
                  {quickViewProduct.regularPrice && (
                    <span className="text-sm text-[#736E9B] line-through">
                      {quickViewProduct.regularPrice}
                    </span>
                  )}
                </div>
              ...
              <div className="mt-6 flex items-center gap-3">
                <Link
                  ...
                  className="flex-1 bg-[#FF4D6D] hover:bg-[#ff3358] text-white text-center font-bold text-sm py-3 rounded-full shadow-sm active:scale-95 transition-all"
                >
                  Full Details
                </Link>
                <button
                  onClick={() => setQuickViewProduct(null)}
                  className="px-4 py-3 border border-[#EAE3F7] rounded-full text-xs font-semibold text-[#736E9B] hover:text-[#171136]"
                >
                  Close
                </button>
              </div>
